...
**The clock** lives on `#clock-platform`, anchored to the bar's top-right corner (`position:absolute;top:0;right:0`, inside `#top-menubar` which is `position:fixed` and gives it its containing block) — replacing the side panel's rotated `.tod-box` clock. `#mb-clock` is set 5x the size of a normal bar label, too tall to fit inside `var(--menubar-h)` without changing the bar's own height, so rather than shrink it back down, `#clock-platform` is allowed to grow past the bar's bottom edge and hang down over the felt's top-right corner below. It's taken out of flex flow (`position:absolute`) so it doesn't disturb the Game/Pause/Payout/Help layout. It used to sit on a raised wood pedestal (its own background/border/`border-radius`, routing the bar's `border-bottom` around it); that's been removed, so `#mb-clock`'s numerals now float directly over whatever's behind them, at 33% `opacity` so the clock reads as secondary to the timer itself. Styled as a retro LED display — a monospace font stack (no vendored/network font, same constraint as the rest of the app) standing in for a segmented digital face, plus a red `color` and blurred red `text-shadow` glow for the backlight effect — rather than the app's usual white numerals.
