...
- *For the first `FLIP_DELAY_SEC` (15) seconds of a manually-picked level:* `snapToRTC()` means `remSec` right after picking a level is rarely a clean number (e.g. 15:13 instead of 15:00), which reads as broken to anyone who doesn't know about the cross-device alignment. For `FLIP_DELAY_SEC` seconds of wall-clock time since `levelStartWall` (set in `beginCountdown()`, i.e. only on a manual pick from the menu — not on resume, not on `startSynced()`), it shows a plain countdown from `maxSec(level)` instead of the true `remSec`, then "flips" over to the real synced value once that window elapses. Any manual `adjustLevel()`/`adjustTime()` input clears `levelStartWall`, flipping to the real value immediately instead of waiting out the rest of the window.
...
Either way, auto-advance, the wood-clack/alarm-arming checks in `tick()`, and cross-device sync all still key off the true `remSec` throughout.
...
**The clock** lives on `#clock-platform`, a child of `#cd-zone` sitting in the exact spot the felt's heart glyph used to occupy — `position:absolute;left:2vw` matches `.suit-clubs`' own `left:2vw` (`#cd-zone` is already `position:relative`, same as `.suit`'s own containing zones) for left-edge alignment; `bottom:4.78vh` (rather than plumbing `.suit-spades`' own `bottom:2vh` value directly) corrects for the two elements' different bottom padding — a `.suit` glyph is a plain text character, so its glyph ink sits above its box's true bottom edge by the font's descent metrics, while the seven-segment digits' own bottom segment is drawn flush to `.ssd-digit`'s bottom edge. `4.78vh` was reverse-engineered from a pixel-level comparison (a real headless-browser screenshot, scanning for each glyph's lowest non-background pixel) rather than guessed, and confirmed to hold at more than one viewport size since both offsets are `vh`-relative. The result: the clock's visible ink lines up with `.suit-clubs` on the left and `.suit-spades` on the bottom, as if it were the fourth corner glyph. `z-index:-1` matches `.suit`'s own stacking, so it paints behind the countdown digits the same way the glyphs do when they overlap; `pointer-events:none` so it never intercepts the felt's swipe/click zones underneath it. Because it now lives inside `#cd-zone` rather than the shared bar chrome, it only shows on the running-timer screen, not the menu screen (which has no suit glyphs to align with either). `#mb-clock` is set 5x the size of a normal bar label. No background/border of its own (it used to sit on a raised wood pedestal; that's been removed), so `#mb-clock`'s numerals just float over the felt, at full `opacity` and no glow.

**The clock face is a true seven-segment display, not a font.** There's no system-installed 7-segment font to name, and adding one would mean either a network fetch or vendoring a font file — both against the single-file/offline-only design. `renderSSD()` (Clock section of the script) turns each character of `fmtClock()`'s `"H:MM"` string into a `.ssd-digit` or `.ssd-colon` `<span>`; `SSD_SEGMENTS` maps each digit `0`-`9` to a string of which of the seven segments (`a`-`g`, standard naming — `a`=top, `b`/`c`=upper/lower-right, `d`=bottom, `e`/`f`=lower/upper-left, `g`=middle) are lit, and `renderSSD()` emits one absolutely-positioned `.ssd-seg.seg-<letter>` `<i>` per lit segment (unlit segments aren't rendered at all — no dim "ghost" segments), each a plain solid-black rounded bar with no `text-shadow`. Segment thickness (`.seg-a`/`.seg-g`/`.seg-d`'s `height`, `.seg-b`/`.seg-c`/`.seg-e`/`.seg-f`'s `width`) and the horizontal bars' side inset are both derived from the same value so the bars still meet cleanly at the corners — bumping one without the other reintroduces gaps or overlap at the joints. `updateClocks()` calls `elMbClock.innerHTML=renderSSD(fmtClock())` on its existing 1s heartbeat. `fmtTime()`/`fmtClock()` themselves are untouched and still back the plain-text pause label (`updatePauseLabel()`) — only the corner clock's own rendering goes through `renderSSD()`.
