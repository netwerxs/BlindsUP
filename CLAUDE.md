...
| `inBreak` / `breakRemSec` / `breakEndWall` / `breakTO` | Break countdown state — `breakRemSec` is derived from `breakEndWall` each tick, not decremented directly |
...
**Break is not a level.** It auto-starts the instant level 5 finishes (`tick()`: `if(level===5){level=6;startBreak();}`) — there is no manual Break button. `startBreak()` sets `inBreak`, freezes the level timer via `freezeCountdown()`, plays a brief full-screen color-flash (`flashBreak()`), then shows the menu with the Resume button relabeled `On Break MM:SS`. `tickBreak()` derives `breakRemSec` from `breakEndWall` (wall-clock-based, not a naive per-second decrement, so background throttling/sleep can't make the break run long) and fires the horn (`playAlarm()`) once each as remaining time crosses 3min, 1min, and 0 (tracked via `breakAlarmed180/60/0` so a delayed/throttled tick can't skip or double-fire a horn). At zero the Resume button reads `Let's play, good luck!`. A break ends either by picking a new level from the menu (`selectBlind()`) or via the Esc-hold reset — both explicitly clear `inBreak`/`breakTO`.
...
Dark, high-contrast, glanceable-from-across-the-table is the load-bearing constraint — the blind and countdown numerals never get blur, gradients, or texture. "2026" polish (translucent `backdrop-filter` panels, soft shadows, inset highlights, the menu grid's current-level glow) is confined to chrome (`#col-right`, `.menu-corner-btn`, `.pill-btn`, `.blind-card`) via the `--glass-*`/`--shadow-*`/`--inset-hi` tokens in `:root`, never to the data itself. A global `prefers-reduced-motion` media query collapses all animation/transition durations for users who request it.
