...
**Display vs. real remaining time, for the first 25s of a manually-picked level.** `snapToRTC()` means `remSec` right after picking a level is rarely a clean number (e.g. 15:13 instead of 15:00), which reads as broken to anyone who doesn't know about the cross-device alignment. `displayRemSec()` (used only by `render()`, for the `#cdMin`/`#cdSec` text) papers over this: for `FLIP_DELAY_SEC` (25) seconds of wall-clock time since `levelStartWall` (set in `beginCountdown()`, i.e. only on a manual pick from the menu — not on resume, not on `startSynced()`), it shows a plain countdown from `maxSec(level)` instead of the true `remSec`, then "flips" over to the real synced value once that window elapses. `remSec` itself is never touched by this — auto-advance, the wood-clack/alarm-arming checks in `tick()`, and cross-device sync all still key off the true value throughout; only the displayed digits lie for those first 25 seconds.
