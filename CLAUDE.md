...
**The clock** lives on `#clock-platform`, a child of `#cd-zone` (already `position:relative`) — a small white-felt plaque tucked flush into the felt's own bottom-left corner (`position:absolute;bottom:0;left:0`), the spot the felt's heart glyph used to occupy. Its bottom and left edges sit flush against the real rail (the wood `.felt-frame` border) there, so they carry no border of their own; the top and right edges — which face the open felt instead of the rail — get an actual mini rail of their own, via `border-image-source` set to the same wood-grain `linear-gradient` tones as `.wood-layout` (`border-width:0.6vh 0.6vh 0 0` scopes it to just those two sides; `border-image-slice:1` stretches the gradient across them). `border-radius` rounds all four corners of the box; the bottom-left one is moot in practice since `#felt-surface`'s own `overflow:hidden` already clips to its matching corner there, but rounding it too keeps the box's geometry consistent regardless. `z-index:-1` matches `.suit`'s own stacking, so it paints behind the countdown digits if they ever overlap; `pointer-events:none` so it never intercepts the felt's swipe/click zones underneath it. Because it lives inside `#cd-zone` rather than the shared bar chrome, it only shows on the running-timer screen, not the menu screen. `#mb-clock` is set 5x the size of a normal bar label. Its black seven-segment digits sit on the plaque's white background at full `opacity` and no glow, reading like an inset digital display built into the felt rather than floating numerals.
