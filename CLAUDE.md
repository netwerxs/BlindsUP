...
`fmtBlind(v, lv)` is the single display formatter for blind values: raw digits through level 13, abbreviated to `Nk` (lowercase, whole thousands) from level 14 on, with the `k` wrapped in a `.blind-k` span. It returns an **HTML fragment**, so every consumer assigns it via `innerHTML` — the countdown screen, menu cards, the level-up announce, the resume button. The `k` is shrunk to `0.7em` (30% smaller) **only on the big countdown numerals** (`.b-num .blind-k`, `.ann-num .blind-k`); on the picker grid and resume button it stays full size. The `BLINDS` array itself always holds the full integer values.
