...
**The clock and version** live at the right edge of the bar (`#mb-right`, pushed there with `margin-left:auto`) — `#mb-clock` (plain horizontal text, unlike the old side panel's rotated one) and `#mb-version`, replacing the side panel's rotated `.tod-box` clock and the old per-screen version labels.
