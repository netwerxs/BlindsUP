...
**The clock** lives on `#clock-platform`, its own top-level `position:fixed;left:0;bottom:0` element (a sibling of `#bottom-bar`, not nested inside `#top-menubar`) anchored to the screen's bottom-left corner — the spot the felt's heart glyph used to occupy (see `#cd-zone` below; the heart was removed to make room for it, and its `.suit-hearts` CSS rule removed along with it). `position:fixed` plus `z-index:41` (matching the rest of the chrome) keeps it shared and visible on both screens without needing to live inside either bar, the same way `#bottom-bar`'s version label works; `pointer-events:none` so it never intercepts the felt's swipe/click zones underneath it. `#mb-clock` is set 5x the size of a normal bar label — free to sit at that size since it's not constrained by any bar's own height. No background/border of its own (it used to sit on a raised wood pedestal; that's been removed), so `#mb-clock`'s numerals just float over whatever's behind them, at 33% `opacity` so the clock reads as secondary to the timer itself. Styled as a retro LED display — a monospace font stack (no vendored/network font, same constraint as the rest of the app) standing in for a segmented digital face, plus a red `color` and a two-layer blurred red `text-shadow` glow for the backlight effect — rather than the app's usual white numerals.
