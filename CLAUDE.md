...
**The clock** lives on `#clock-platform`, a plain rectangular wood pedestal, `height:2in`, sitting flush in the top-right corner (`position:absolute;top:0;right:0`, inside `#top-menubar` which is `position:fixed` and gives it its containing block) — replacing the side panel's rotated `.tod-box` clock. `#mb-clock` is set 5x the size of a normal bar label, too tall to fit inside `var(--menubar-h)` without changing the bar's own height, which is why it needs the pedestal rather than living in the bar directly. Think of the felt rectangle as having a border tracing its full perimeter; the pedestal sits flush against the screen's own top and right edges (no border needed on those two sides — that's just the screen's corner), so the border only needs to run along the pedestal's other two sides: `border-left` and `border-bottom`, meeting at a plain sharp corner — no `border-radius`, no curve. It's taken out of flex flow (`position:absolute`) so it doesn't disturb the Game/Utils/Help layout.
