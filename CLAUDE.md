...
**The clock and version** live on `#clock-platform`, a raised wood plaque anchored to the bar's top-right corner (`position:absolute;top:0;right:0`, inside `#top-menubar` which is `position:fixed` and gives it its containing block) — replacing the side panel's rotated `.tod-box` clock and the old per-screen version labels. `#mb-clock` is set 5x the size of a normal bar label, too tall to fit inside `var(--menubar-h)` without changing the bar's own height, so rather than shrink it back down, the platform is allowed to grow past the bar's bottom edge and hang down over the felt's top-right corner below (past the wood border's own thickness), like a plaque mounted at the corner instead of a label squeezed into the bar. It's taken out of flex flow (`position:absolute`) so it doesn't disturb the Game/Utils/Help layout, and only its bottom-left corner is rounded, reading as flush with the screen's top and right edges.
